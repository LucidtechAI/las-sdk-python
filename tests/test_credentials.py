...
def mock_read_from_environ(*args, **kwargs):
    return ['foo', 'bar', 'baz', 'foobar', 'foobaz'], None


def test_guess_credentials():
    (client_id, client_secret, api_key, auth_endpoint, api_endpoint), _ = mock_read_from_environ()
    ...
    with pytest.MonkeyPatch().context() as mp:
        ...
        assert api_key == credentials.api_key
